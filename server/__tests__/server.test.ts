...
describe('loadConfig', () => {
  ...
  it('returns defaults when only MESH_ADMIN_TOKEN is set', async () => {
    ...
    expect(config).toEqual({ dbPath: '/data/mesh.db', wsPort: 7384, adminPort: 7385, adminToken: 'tok', cleanupIntervalMs: 60000, maxFileBytes: 10_485_760 });
  });
  ...
  it('returns correct values when all valid env vars are set', async () => {
    ...
    expect(exitCode).toBeUndefined();
    expect(config).toEqual({ dbPath: '/tmp/test.db', wsPort: 8080, adminPort: 7385, adminToken: 'secret', cleanupIntervalMs: 60000, maxFileBytes: 10_485_760 });
  });
