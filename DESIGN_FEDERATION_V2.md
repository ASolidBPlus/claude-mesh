# DESIGN v2.2: Mesh-to-Mesh Federation

**Status: PLAN — nothing here is implemented.** Plan-only PR; implementation starts only on the owner's explicit approval, then as separate PRs (§11). **Supersedes `DESIGN_FEDERATION.md`** (single-bus multi-tenant, scrapped by the owner the day it merged: *"why else would we do this if it's all on one bus?"*). v2.2 is the respin after two evaluator rounds (NO-GO, then NO-GO-narrow): every mechanism below was checked against main `74771ee` (and the in-lane #89/#90/#95 branches where named), and every point a builder would otherwise have had to decide is decided here (§14 ledger, §15 round-2 resolutions).
...
--   Runs OUTSIDE any db.transaction() (PRAGMA foreign_keys=OFF is a no-op inside a transaction — verified); idempotent via a probe: rebuild only if
--   PRAGMA foreign_key_list(acl) is non-empty. Steps: foreign_keys=OFF; CREATE TABLE acl_new(... no FKs ...); INSERT ... SELECT; DROP acl; RENAME; recreate every acl index
--   present at that time (idx_acl_reverse lands with #89, which precedes F0); foreign_keys=ON. deleteAgent() then deletes the agent's acl rows EXPLICITLY (it relied on
...
- `POST /outbound-peers {alias, url, token, assigned_alias, kinds?}` (url must be `ws://` or `wss://`; loopback allowed) / `DELETE /outbound-peers/:alias` / `GET /outbound-peers` (alias, url, assigned_alias, enabled, last_alive — **never** token, C7). `PATCH /outbound-peers/:alias {enabled?, token?, url?}` — `token` and `url` are how a receiver-side token rotation or move is applied WITHOUT `DELETE` (which would expire queued mail, §5.6); the forwarder restarts with the new credential. Start/stop/restart is event-driven from these handlers, never polled.
...
- `MeshClient`: `rawSend` and `sendWithAck` become `protected` (they are `private` today, so a subclass cannot reach them); a new `protected authExtras(): Record<string, unknown>` hook (default `{}`) is merged into the auth frame where it is built; `PROTOCOL_MISMATCH` is added to `onError`'s fatal branch (today only first-auth `AUTH_FAILED` is fatal). `PeerClient extends MeshClient`: `authExtras()` returns `{protocol: 1}`; `relay(frame): Promise<void>` over `sendWithAck` keyed by the relay `msg_id`, rejecting with the peer's `code` (`RELAY_REFUSED`/`RATE_LIMITED`) or the SDK's (`ACK_TIMEOUT`/`CONNECTION_RESET`); `AUTH_FAILED` fatal at any time. Heartbeat/half-open/reconnect unchanged. Agent auth is byte-identical (the hook adds nothing for `MeshClient`); the runtime's SDK pin is unaffected.
