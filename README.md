...
- **Schema changes must be safe to run unattended, because a restart is a deploy.** Boot-time migrations run with no operator watching (host reboots included), so each one must be idempotent and must leave a crash mid-way in a state the next boot repairs. The F0 `acl` rebuild (#97) is the reference shape: the `PRAGMA foreign_keys=OFF` is set outside any transaction (it is a no-op inside one), the DDL and the index replay run inside one explicit transaction, and the FK-probe makes a second run a no-op. A process death after `DROP TABLE acl` rolls back and the next boot retries — tested by injecting a crash at that point. A migration that cannot state this property does not merge.
