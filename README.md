...
**ACL gates delivery.** An ACL entry `from → to` permits `from` to send to `to`. Direct messages, file sends and per-subscriber topic delivery are all checked server-side — but they FAIL DIFFERENTLY, and the difference matters when you are debugging a message that did not arrive. A direct send or file send to an agent you may not reach is **refused**, with `ACL_DENIED` returned to the sender. A **topic publish always succeeds from the publisher's point of view**: it is accepted, and each subscriber is filtered server-side, silently. The publisher is not told which subscribers were dropped, or that any were. That is deliberate — telling the publisher would disclose the topic's subscriber list and the ACL edges between them, which is the class of leak uniform refusal exists to prevent, and even an aggregate ("delivered to n of m") leaks a subscriber count. The drops are counted server-side, in a metric that names no subscriber. **So: never infer from the absence of an error that a topic message was delivered.** ACLs are admin-managed (`POST/DELETE/GET /acl`) — a node can't grant itself access.
...
- `→ list_presence` `{ "type": "list_presence", "msg_id": "p-1" }` → `← presence_list` `{ "type": "presence_list", "ref": "p-1", "agents": [{ "id": "bob", "online": true, "last_seen": ..., "last_alive": ..., "last_responded": ... }] }` (ACL-filtered to you + your peers).
  ...
  **The four readings, and what each actually claims.** (The canonical
  definition lives beside the code, at `server/db.ts`'s `touchResponded`; this
  is its expansion for operators, not a second authority.) They answer different questions and are deliberately not collapsed into one another:
...
- **Counters:** `mesh_messages_total{kind,status}`, `mesh_messages_sent_total{from_agent}`, `mesh_messages_received_total{to_agent}`, `mesh_acl_denied_total{from_agent}` (direct sends only), `mesh_topic_fanout_total{outcome}` (topic fan-out; see [`docs/FEDERATION.md`](docs/FEDERATION.md) §4 for which counter attributes what), `mesh_errors_total{error_code}`, `mesh_bytes_total{direction}`, `mesh_files_total`, `mesh_reminders_fired_total`.
...
| `MESH_METRICS_IDENTITY_LABELS` | *(unset = off)* | `1` emits the **identity-bearing** metric labels: agent ids (`mesh_agent_up{agent}`, `mesh_messages_sent_total{from_agent}`, `mesh_messages_received_total{to_agent}`, `mesh_acl_denied_total{from_agent}`) and peer aliases (`mesh_peer_up{alias}`, `mesh_peer_relays_total{alias,…}`). Off by default because `/metrics` is **unauthenticated**: those labels enumerate the agent roster that `list_presence` ACL-filters, and the federation topology that peer aliases name. A label is the same information as an API response with no auth in front of it. Turning it on **is the operator's acceptance** that the admin port is genuinely internal-only. Default emits aggregates (`mesh_agent_up_count{state}`, `mesh_peer_up_count{state}`, and unlabelled totals) which stay alertable without naming anyone. `mesh_errors_total{error_code}` carries no identity and is unaffected. | **Break-glass:** attributing a flood needs identity labels temporarily, and **what the flag can and cannot attribute differs by class** — direct sends yes, topic fan-out not at all. The procedure and that table have ONE home: [`docs/FEDERATION.md`](docs/FEDERATION.md) §4 *Break-glass: attributing a flood*. Not restated here, deliberately — this row and that table drifted apart once already, and the copy an operator reads mid-incident is the one that went stale. Turning it on is a deploy action and a deliberate, time-boxed exposure: while set, `/metrics` hands the full registered roster to any unauthenticated reader — which is a property of THIS FLAG, so it stays here. |
| `MESH_ADMIN_BIND` | *(unset = all interfaces)* | Bind address for the **admin** listener (`/metrics`, admin API). Default is unchanged — every interface — deliberately, because a loopback default would make the mesh unreachable from the rest of a container network. At boot the server logs `evt:"admin.listening"` naming the bind and stating that `/metrics` is unauthenticated on it, **including when unset**. That log is what upgrades the C9 `/metrics` exemption from an assumption nobody can check to a deployment decision you can see: when the bind is not loopback or otherwise internal, the exemption is **the deployer's acceptance**. |
| `MESH_WS_BIND` | *(unset = all interfaces)* | Bind address for the **agent/peer** WS listener. Separate from `MESH_ADMIN_BIND` because the ports have different audiences: the WS port must be reachable by every agent and peering, the admin port need only be reachable by operators and the spawner stack. One variable for both would force the more permissive. |
...
| `MESH_PLAINTEXT_PEER_CIDRS` | *(unset = loopback only)* | Comma-separated CIDRs (IPv4 and IPv6) where a plaintext `ws://` **outbound** peering is permitted, e.g. `10.20.0.0/16,fd00::/8`. Unset or empty is the old rule exactly: `ws://` for loopback only. Only **literal addresses** are accepted for `ws://` — a hostname is refused even when it resolves in range, because the check and the dial resolve separately (DNS rebinding would put the peering token on the wire in cleartext wherever the name later points). IPv4 is matched in its v4-mapped IPv6 form, so `::ffff:10.20.0.5` matches `10.20.0.0/16`. An unreadable entry, one with host bits set (`10.20.0.5/16`), or a `/0` (plaintext to every address — the rule switched off, not a network) **refuses to start**, naming it. Checked at `POST`/`PATCH /outbound-peers` **and** when a forwarder dials, so narrowing the list (a restart) stops existing peerings too; a refused dial logs `border.link_down` with `refused by policy: …`. At boot the server logs `evt:"peering.plaintext_policy"` with the list in its normalised form, **including `cidrs: []` when unset**. `wss://` is unaffected. |
| `MESH_TLS_CERT` | *(unset = plain HTTP)* | Certificate for the **WS listener**: a path, or the PEM itself (a value starting `-----BEGIN` is content; literal `\n` sequences become newlines). With `MESH_TLS_KEY` set too, the WS port serves **TLS**, and `/healthz`, every agent and every peering cross it together. **Every client of that bus, agents and services as well as peers, must then dial `wss://` and trust the CA.** Setting only one of the pair **refuses to start**, naming the missing one, rather than falling back to plaintext. An unreadable or invalid certificate or key, or a key that does not match the certificate, also refuses to start. `ws.listening` carries `tls: true\|false` (printed when off too), and when on, the certificate's subject, SANs and `not_after`. A certificate that has expired, or expires within 14 days, logs `evt:"ws.tls_cert_expiry"` at boot. **No hot reload:** the pair is read once, and rotating it is a restart. The **admin** listener is never TLS; bind it to loopback with `MESH_ADMIN_BIND`. |
| `MESH_TLS_KEY` | *(unset)* | Private key for `MESH_TLS_CERT`: a path or the PEM, same rule. Its content never appears in a log, an error or the boot line; errors name the variable and, at most, the path. |
| `MESH_TLS_CA` | *(unset = runtime defaults)* | CA certificate(s) the **border** verifies the peers it dials against: a path or the PEM, same rule. It **replaces** the default trust store for those connections, so the bus trusts exactly this CA for peering. Independent of serving TLS. Verifying IP-literal peers needs Bun ≥ 1.4 (see the client README); the image is built on that floor. |
