...
- **Counters:** `mesh_messages_total{kind,status}`, `mesh_messages_sent_total{from_agent}`, `mesh_messages_received_total{to_agent}`, `mesh_acl_denied_total{from_agent}` (DIRECT sends only), `mesh_topic_fanout_total{outcome}` (per-subscriber ACL outcomes of topic fan-out, **no party dimension**), `mesh_errors_total{error_code}`, `mesh_bytes_total{direction}`, `mesh_files_total`, `mesh_reminders_fired_total`.
...
| `MESH_METRICS_IDENTITY_LABELS` | *(unset = off)* | `1` emits the **identity-bearing** metric labels: agent ids (`mesh_agent_up{agent}`, `mesh_messages_sent_total{from_agent}`, `mesh_messages_received_total{to_agent}`, `mesh_acl_denied_total{from_agent}`) and peer aliases (`mesh_peer_up{alias}`, `mesh_peer_relays_total{alias,…}`). Off by default because `/metrics` is **unauthenticated**: those labels enumerate the agent roster that `list_presence` ACL-filters, and the federation topology that peer aliases name. A label is the same information as an API response with no auth in front of it. Turning it on **is the operator's acceptance** that the admin port is genuinely internal-only. Default emits aggregates (`mesh_agent_up_count{state}`, `mesh_peer_up_count{state}`, and unlabelled totals) which stay alertable without naming anyone. `mesh_errors_total{error_code}` carries no identity and is unaffected. | **Break-glass, and what it can attribute:** setting this to `1`, restarting, reading, then **unsetting and restarting again** names the sources of **direct** sends refused by ACL, via `mesh_acl_denied_total{from_agent}`. It does **nothing** for topic fan-out: `mesh_topic_fanout_total` has no party dimension **by construction** — absent, not hidden — because topic names and subscriber lists are agent-chosen and a label carrying one would put an agent-chosen string into unauthenticated `/metrics`. **The flag cannot reveal what is not recorded**, so for a fan-out flood this costs two restarts and an exposure window and returns nothing; see [`docs/FEDERATION.md`](docs/FEDERATION.md) §4 for that route. It is a deploy action, not a code change, and it is a deliberate time-boxed exposure: while set, `/metrics` hands the full registered roster to any unauthenticated reader. See [`docs/FEDERATION.md`](docs/FEDERATION.md) §4. |
